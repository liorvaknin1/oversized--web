# Security Policy (internal) — OBSIZE

> **Internal, unpublished version.** Kept in git but stripped from the
> deployed site (see `.github/workflows/deploy.yml`). The public-facing,
> generic policy lives in `SECURITY.md`. This file holds operational and
> edge-configuration details that must not be exposed publicly.
